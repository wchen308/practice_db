...
/* window function
rank change_date before 2019-08-16 DESC by product_id
return rank = 1
*/

WITH ranked_db AS (SELECT product_id, new_price, RANK() OVER (PARTITION BY product_id ORDER BY change_date DESC) AS rnk
                   FROM Products
                   WHERE change_date <= '2019-08-16')
,
product_db AS (SELECT DISTINCT product_id FROM Products)
,
latest_price AS (SELECT product_id, new_price
                 FROM ranked_db
                 WHERE rnk = 1)

SELECT p.product_id, IFNULL(l.new_price, 10) AS price
FROM product_db AS p LEFT JOIN latest_price AS l
